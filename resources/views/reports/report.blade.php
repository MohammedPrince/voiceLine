...
<div class="reports-container">
  ...
  color: #ec8305;">Voice Calls Report</h5>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body" style="padding: 20px;">
                ...
                <div id="statusContent" style="display:none;">
                    ...
                    <p>
                        <b>CGPA:</b> <span id="studentCgpa">{{ session('last_cgpa') }}</span>
                        &nbsp; | &nbsp;
                        <b>Status:</b> <span id="studentStatus">{{ session('status') }}</span>
                    </p>

                    <!-- F/Z/I Subjects Table -->
                    <div class="row mb-2">
...
// ============================================================================
// Add this helper function near the top of your script, before renderTablePage
function val(v) {
    return (v !== null && v !== undefined && v !== '') ? v : '<span class="text-muted">Not set</span>';
}
function renderTablePage(page) {
    ...
    // Build Rows with data-label attributes for mobile view
    let rowsHtml = '';
    paginatedItems.forEach(row => {
        let fullData = encodeURIComponent(JSON.stringify(row));
        rowsHtml += `
    <tr>
        <td data-label="Staff"><span class="fw-bold text-muted">${val(row.handled_by)}</span></td>
        <td data-label="Ticket">${val(row.ticket_number)}</td>
        <td data-label="Category"><small>${val(row.category)}</small></td>
        <td data-label="Final Status">${getStatusBadge(row.Final_Status)}</td>
        <td data-label="Std-index">${val(row.stud_id)}</td>
        <td data-label="Details" class="d-flex gap-1">
